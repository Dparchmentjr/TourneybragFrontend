...
class LoginForm extends React.Component {
    ...
    isValidLogin(data) {
        const {errors, feedback, valid} = Validate(data);
        if (!valid) {
    ...
    handleLogin(e) {
        ...
        if(this.isValidLogin(this.state)) {
            ...
            this.props.login(this.state).then(
                (loggedin) => console.log("Logging in!"),
                (notLoggedIn) => this.setState({errors: notLoggedIn.errors, feedback: notLoggedIn.feedback})
